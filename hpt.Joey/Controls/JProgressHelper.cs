...
{
    public class JProgressHelper
    {
        private BackgroundWorker _backgroundWorker;//后台线程
        private JProcess _processForm;//进度条窗体
        private BackgroundWorkerEventArgs _eventArgs;//异常参数
        private string _inforMessage;

        public bool ShowProgress
        {
            get => _processForm.ShowProgress;
            set => _processForm.ShowProgress = value;
        }
        public JProgressHelper()
        {
            _processForm = new JProcess();
        ...
        {
            _processForm.MessageInfo = _inforMessage;
        ...
        private void _backgroundWorker_DoWork(object sender, DoWorkEventArgs e)
        {
            if (BackgroundWork != null)
            {
                try
                {
                    BackgroundWork(this.ReportPercent, ReportMessage);
                }
                catch (Exception ex)
                {
        ...
        /// </example></para>
        /// </summary>
        public Action<Action<int>, Action<string>> BackgroundWork { get; set; }
        ...
        {
            _processForm.CurrentMessage = msg;
        }
    }
}
