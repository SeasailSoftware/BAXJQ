...
namespace HPT.Face.AXD
{
    public class HaCamera
    {
        ...
        /// 连接状态
        /// </summary>
        public bool Connected => NativeMethods.HA_Connected(_cam);
        ...
        public bool AddFace(string personID, string personName, int personRole, byte[] picData, uint wgNo, uint effectTime, out string msg)
        {
            lastErrorCode = NativeConstants.ERR_NONE;
            if (picData == null)
            {
                lastErrorCode = NativeConstants.ERR_INVALID_PARAM;
                msg = GetErrorDescribe(lastErrorCode);
                return false;
            }
            if (string.IsNullOrEmpty(personID))
            {
        ...
        /// <param name="timeOutInMilli">超时时间</param>
        /// <returns>设备实时画面截图；返回null表示截图失败</returns>
        public Image Snapshot(int timeOutInMilli, out string msg)
        {
            lastErrorCode = NativeConstants.ERR_NONE;
            _snapshotImage = null;
            int ret = NativeMethods.HA_Snapshot(_cam);
